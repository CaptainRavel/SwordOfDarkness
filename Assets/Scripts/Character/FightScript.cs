...
public class FightScript : MonoBehaviour
{
    ...
    public void POdczytPO()
    {
        ...
        if ((int)Random.Range(1, 101) <= punikiprocent)
        {
            wiempu = true;
            wiempp = false;
            wiempg = false;
        ...
        {
            if ((int)Random.Range(1, 101) <= pparowanieprocent)
            {
                wiempu = false;
                wiempp = true;
                wiempg = false;
    ...
    {
        unikiPOprzeciwnika = Enemy1Script.e1unikiPO;
        gardaPOprzeciwnika = Enemy1Script.e1gardaPO;
        parowaniePOprzeciwnika = Enemy1Script.e1parowaniePO;
    ...
    public void PObliczanieTrafienia()
    {
        ...
        {
            eszansatrafienia = minK20 + (int)E1ModifiScript.refleks;
            Debug.Log("Wybrany modyfikator refleksu(uniki): " + (int)E1ModifiScript.refleks);
        }
        ...
        if (ewylosowanaobrona == "Garda")
        {
            ...
            eszansatrafienia = minK20 + (int)E1ModifiScript.wytrzymalosc;
            Debug.Log("Wybrany modyfikator wytrzymałości(parowanie): " + (int)E1ModifiScript.wytrzymalosc);
        }
    ...
    public void PAtak()
    {
        ...
        if (ptrafienie == 1)
        {
            ...
            if (ewylosowanaobrona == "Garda")
            {
                pobrazenia = (int)(PModifiScript.obrazenia - E1ModifiScript.wytrzymalosc);
                if (pobrazenia <= 0)
                {
                    pobrazenia = 0;
                }
                int pobrazeniakrytyczne = pobrazenia * 2;
                int pobrazeniazredukowane = pobrazenia - (int)(pobrazenia * Enemy1Script.e1gardepercent);
                int pobrazeniakrytycznezredukowane = pobrazeniakrytyczne - (int)(pobrazeniakrytyczne * Enemy1Script.e1gardepercent);
                int czykrytyk = 2;
                if (Random.Range(1, 101) <= czykrytyk)
                {
                    ...
                    {
                        Enemy1Script.e1currentHealth = Enemy1Script.e1currentHealth - pobrazeniakrytycznezredukowane;
                        PlayerBattle.pcurrkon = PlayerBattle.pcurrkon - 15;
                        info.text += "<Color=blue>" + "<b>" + "Wykonujesz atak i zadajesz uderzeniem krytycznym " + pobrazeniakrytycznezredukowane + " pkt obrażeń! (Przeciwnik zredukował gardą" + Enemy1Script.e1garde + "% obrażeń)" + "\n" + "</b>" + "</color>";
                    ...
                    else
                    {
                        ...
                        {
                            Enemy1Script.e1currentHealth = Enemy1Script.e1currentHealth - pobrazeniazredukowane;
                            PlayerBattle.pcurrkon = PlayerBattle.pcurrkon - 5;
                            info.text += "<Color=blue>" + "<b>" + "\n" + "Wykonujesz atak i zadajesz " + pobrazeniazredukowane + " pkt obrażeń! (Przeciwnik zredukował gardą " + Enemy1Script.e1garde + "% obrażeń)" + "\n" + "\n" + "</b>" + "</color>";
            ...
            else
            {
                ...
                if (Random.Range(1, 101) <= czykrytyk)
                {
                    ...
                    if (PlayerBattle.pcurrkon >= czymakondycje)
                    {
                        Enemy1Script.e1currentHealth = Enemy1Script.e1currentHealth - pobrazeniakrytyczne;
                        PlayerBattle.pcurrkon = PlayerBattle.pcurrkon - 15;
                    ...
                    else
                    {
                        if (PlayerBattle.pcurrkon >= 5)
                        {
                            Enemy1Script.e1currentHealth = Enemy1Script.e1currentHealth - pobrazenia;
                            PlayerBattle.pcurrkon = PlayerBattle.pcurrkon - 5;
    ...
    {
        eatak = 1;
        eszansatrafienia = rzut1k20 + E1ModifiScript.trafienie;
        info.text += "<Color=red>" + "Punkty trafienia przeciwnika: " + rzut1k20 + "+" + E1ModifiScript.trafienie + "=" + eszansatrafienia + "\n" + "</color>";
    ...
    public void EAtak()
    {
        ...
        if (etrafienie == 1)
        {
            ...
            if (pwylosowanaobrona == "Garda")
            {
                eobrazenia = (int)(E1ModifiScript.obrazenia - PModifiScript.wytrzymalosc);
                if (eobrazenia <= 0)
                {
                    eobrazenia = 0;
                }
                ...
                if (Random.Range(1, 101) <= czykrytyk)
                {
                    ...
                    czyemakondycje = Enemy1Script.e1currentKon * 0.03f;
                    if (Enemy1Script.e1currentKon >= czyemakondycje)
                    {
                        PlayerBattle.pcurrhealth = PlayerBattle.pcurrhealth - eobrazeniakrytycznezredukowane;
                        Enemy1Script.e1currentKon = Enemy1Script.e1currentKon - 15;
                    ...
                    {
                        if (Enemy1Script.e1currentKon >= 5)
                        {
                            PlayerBattle.pcurrhealth = PlayerBattle.pcurrhealth - eobrazeniazredukowane;
                            Enemy1Script.e1currentKon = Enemy1Script.e1currentKon - 5;
            ...
            else
            {
                ...
                if (Random.Range(1, 101) <= czykrytyk)
                {
                    float czymakondycje = Enemy1Script.e1currentKon * 0.03f;
                    if (Enemy1Script.e1currentKon >= czymakondycje)
                    {
                    ...
                    if (ewylosowanaobrona == "Garda")
                    {
                        ...
                        if (Random.Range(1, 101) <= czykrytyk)
                        {
                            ...
                            if (PlayerBattle.pcurrkon >= czymakondycje)
                            {
                                Enemy1Script.e1currentHealth = Enemy1Script.e1currentHealth - pobrazeniakrytycznezredukowane;
                                PlayerBattle.pcurrkon = PlayerBattle.pcurrkon - 15;
                                info.text += "<Color=blue>" + "<b>" + "Wykonujesz kontratak i zadajesz uderzeniem krytycznym " + pobrazeniakrytycznezredukowane + " pkt obrażeń! (Przeciwnik zredukował gardą" + Enemy1Script.e1garde + "% obrażeń)" + "\n" + "</b>" + "</color>";
                                Debug.Log("Gracz zadaje " + pobrazeniakrytycznezredukowane + " pkt obrażeń! (redukcja gardą)");
                            }
                            else
                            {
                                if (PlayerBattle.pcurrkon >= 5)
                                {
                                    Enemy1Script.e1currentHealth = Enemy1Script.e1currentHealth - pobrazeniazredukowane;
                                    PlayerBattle.pcurrkon = PlayerBattle.pcurrkon - 5;
                                    info.text += "<Color=blue>" + "<b>" + "\n" + "Wykonujesz kontratak i zadajesz " + pobrazeniazredukowane + " pkt obrażeń! (Przeciwnik zredukował gardą " + Enemy1Script.e1garde + "% obrażeń)" + "\n" + "\n" + "</b>" + "</color>";
                                    Debug.Log("Gracz zadaje " + pobrazeniazredukowane + " pkt obrażeń z kontrataku! (redukcja gardą)");
                                }
                                else
                                {
    ...
    public void EKontratak()
    {
        if (etrafienie == 1)
        {
            if (ewylosowanaobrona == "Parowanie")
            {
                if ((int)Random.Range(1, 101) <= Enemy1Script.e1counter)
                {
                    if (pwylosowanaobrona == "Garda")
                    {
                        eobrazenia = (int)(E1ModifiScript.obrazenia - PModifiScript.wytrzymalosc);
    ...
    public void RegeneracjaKondycji()
    {
        ...
        Enemy1Script.e1currentKon = Enemy1Script.e1currentKon + E1ModifiScript.conditionreg;
