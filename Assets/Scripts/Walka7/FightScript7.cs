...
public class FightScript7 : MonoBehaviour
{
    public Text info;
    ...
    {
        unikiPOprzeciwnika = Enemy7Script.e1unikiPO;
        gardaPOprzeciwnika = Enemy7Script.e1gardaPO;
        parowaniePOprzeciwnika = Enemy7Script.e1parowaniePO;
    ...
    public void PObliczanieTrafienia()
    {
        ...
        {
            eszansatrafienia = minK20 + (int)E7ModifiScript.refleks;
            Debug.Log("Wybrany modyfikator refleksu(uniki): " + (int)E7ModifiScript.refleks);
        }
        ...
        if (ewylosowanaobrona == "Garda")
        {
            ...
            eszansatrafienia = minK20 + (int)E7ModifiScript.wytrzymalosc;
            Debug.Log("Wybrany modyfikator wytrzymałości(parowanie): " + (int)E7ModifiScript.wytrzymalosc);
        }
    ...
    public void PAtak()
    {
        ...
        if (ptrafienie == 1)
        {
            ...
            if (ewylosowanaobrona == "Garda")
            {
                pobrazenia = (int)(PModifiScript.obrazenia - E7ModifiScript.wytrzymalosc);
                if (pobrazenia <= 0)
                {
                    pobrazenia = 0;
                }
                int pobrazeniakrytyczne = pobrazenia * 2;
                int pobrazeniazredukowane = pobrazenia - (int)(pobrazenia * Enemy7Script.e1gardepercent);
                int pobrazeniakrytycznezredukowane = pobrazeniakrytyczne - (int)(pobrazeniakrytyczne * Enemy7Script.e1gardepercent);
                int czykrytyk = 2;
                if (Random.Range(1, 101) <= czykrytyk)
                {
                    ...
                    {
                        Enemy7Script.e1currentHealth = Enemy7Script.e1currentHealth - pobrazeniakrytycznezredukowane;
                        PlayerBattle.pcurrkon = PlayerBattle.pcurrkon - 15;
                        info.text += "<Color=blue>" + "<b>" + "Wykonujesz atak i zadajesz uderzeniem krytycznym " + pobrazeniakrytycznezredukowane + " pkt obrażeń! (Przeciwnik zredukował gardą" + Enemy7Script.e1garde + "% obrażeń)" + "\n" + "</b>" + "</color>";
                    ...
                    else
                    {
                        ...
                        {
                            Enemy7Script.e1currentHealth = Enemy7Script.e1currentHealth - pobrazeniazredukowane;
                            PlayerBattle.pcurrkon = PlayerBattle.pcurrkon - 5;
                            info.text += "<Color=blue>" + "<b>" + "\n" + "Wykonujesz atak i zadajesz " + pobrazeniazredukowane + " pkt obrażeń! (Przeciwnik zredukował gardą " + Enemy7Script.e1garde + "% obrażeń)" + "\n" + "\n" + "</b>" + "</color>";
            ...
            else
            {
                ...
                if (Random.Range(1, 101) <= czykrytyk)
                {
                    ...
                    if (PlayerBattle.pcurrkon >= czymakondycje)
                    {
                        Enemy7Script.e1currentHealth = Enemy7Script.e1currentHealth - pobrazeniakrytyczne;
                        PlayerBattle.pcurrkon = PlayerBattle.pcurrkon - 15;
                    ...
                    else
                    {
                        if (PlayerBattle.pcurrkon >= 5)
                        {
                            Enemy7Script.e1currentHealth = Enemy7Script.e1currentHealth - pobrazenia;
                            PlayerBattle.pcurrkon = PlayerBattle.pcurrkon - 5;
    ...
    {
        eatak = 1;
        eszansatrafienia = rzut1k20 + E7ModifiScript.trafienie;
        info.text += "<Color=red>" + "Punkty trafienia przeciwnika: " + rzut1k20 + "+" + E7ModifiScript.trafienie + "=" + eszansatrafienia + "\n" + "</color>";
     ...
     public void EAtak()
    {
        ...
        if (etrafienie == 1)
        {
            ...
            if (pwylosowanaobrona == "Garda")
            {
                eobrazenia = (int)(E7ModifiScript.obrazenia - PModifiScript.wytrzymalosc);
                if (eobrazenia <= 0)
                {
                    eobrazenia = 0;
                }
                ...
                if (Random.Range(1, 101) <= czykrytyk)
                {
                    ...
                    czyemakondycje = Enemy7Script.e1currentKon * 0.03f;
                    if (Enemy7Script.e1currentKon >= czyemakondycje)
                    {
                        PlayerBattle.pcurrhealth = PlayerBattle.pcurrhealth - eobrazeniakrytycznezredukowane;
                        Enemy7Script.e1currentKon = Enemy7Script.e1currentKon - 15;
                    ...
                    {
                        if (Enemy7Script.e1currentKon >= 5)
                        {
                            PlayerBattle.pcurrhealth = PlayerBattle.pcurrhealth - eobrazeniazredukowane;
                            Enemy7Script.e1currentKon = Enemy7Script.e1currentKon - 5;
            ...
            else
            {
                ...
                if (Random.Range(1, 101) <= czykrytyk)
                {
                    float czymakondycje = Enemy7Script.e1currentKon * 0.03f;
                    if (Enemy7Script.e1currentKon >= czymakondycje)
                    {
                    ...
                    if (ewylosowanaobrona == "Garda")
                    {
                        ...
                        if (Random.Range(1, 101) <= czykrytyk)
                        {
                            ...
                            if (PlayerBattle.pcurrkon >= czymakondycje)
                            {
                                Enemy7Script.e1currentHealth = Enemy7Script.e1currentHealth - pobrazeniakrytycznezredukowane;
                                PlayerBattle.pcurrkon = PlayerBattle.pcurrkon - 15;
                                info.text += "<Color=blue>" + "<b>" + "Wykonujesz kontratak i zadajesz uderzeniem krytycznym " + pobrazeniakrytycznezredukowane + " pkt obrażeń! (Przeciwnik zredukował gardą" + Enemy7Script.e1garde + "% obrażeń)" + "\n" + "</b>" + "</color>";
                                Debug.Log("Gracz zadaje " + pobrazeniakrytycznezredukowane + " pkt obrażeń! (redukcja gardą)");
                            }
                            else
                            {
                                if (PlayerBattle.pcurrkon >= 5)
                                {
                                    Enemy7Script.e1currentHealth = Enemy7Script.e1currentHealth - pobrazeniazredukowane;
                                    PlayerBattle.pcurrkon = PlayerBattle.pcurrkon - 5;
                                    info.text += "<Color=blue>" + "<b>" + "\n" + "Wykonujesz kontratak i zadajesz " + pobrazeniazredukowane + " pkt obrażeń! (Przeciwnik zredukował gardą " + Enemy7Script.e1garde + "% obrażeń)" + "\n" + "\n" + "</b>" + "</color>";
                                    Debug.Log("Gracz zadaje " + pobrazeniazredukowane + " pkt obrażeń z kontrataku! (redukcja gardą)");
                                }
                                else
                                {
    ...
    public void EKontratak()
    {
        if (etrafienie == 1)
        {
            if (ewylosowanaobrona == "Parowanie")
            {
                if ((int)Random.Range(1, 101) <= Enemy7Script.e1counter)
                {
                    if (pwylosowanaobrona == "Garda")
                    {
                        eobrazenia = (int)(E7ModifiScript.obrazenia - PModifiScript.wytrzymalosc);
    ...
    public void RegeneracjaKondycji()
    {
        ...
        Enemy7Script.e1currentKon = Enemy7Script.e1currentKon + E7ModifiScript.conditionreg;
