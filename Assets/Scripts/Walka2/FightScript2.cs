...
public class FightScript2 : MonoBehaviour
{
    public Text info;
    ...
    {
        unikiPOprzeciwnika = Enemy2Script.e1unikiPO;
        gardaPOprzeciwnika = Enemy2Script.e1gardaPO;
        parowaniePOprzeciwnika = Enemy2Script.e1parowaniePO;
    ...
    public void PObliczanieTrafienia()
    {
        ...
        {
            eszansatrafienia = minK20 + (int)E2ModifiScript.refleks;
            Debug.Log("Wybrany modyfikator refleksu(uniki): " + (int)E2ModifiScript.refleks);
        }
        ...
        if (ewylosowanaobrona == "Garda")
        {
            ...
            eszansatrafienia = minK20 + (int)E2ModifiScript.wytrzymalosc;
            Debug.Log("Wybrany modyfikator wytrzymałości(parowanie): " + (int)E2ModifiScript.wytrzymalosc);
        }
    ...
    public void PAtak()
    {
        ...
        if (ptrafienie == 1)
        {
            ...
            if (ewylosowanaobrona == "Garda")
            {
                pobrazenia = (int)(PModifiScript.obrazenia - E2ModifiScript.wytrzymalosc);
                if (pobrazenia <= 0)
                {
                    pobrazenia = 0;
                }
                int pobrazeniakrytyczne = pobrazenia * 2;
                int pobrazeniazredukowane = pobrazenia - (int)(pobrazenia * Enemy2Script.e1gardepercent);
                int pobrazeniakrytycznezredukowane = pobrazeniakrytyczne - (int)(pobrazeniakrytyczne * Enemy2Script.e1gardepercent);
                int czykrytyk = 2;
                if (Random.Range(1, 101) <= czykrytyk)
                {
                    ...
                    {
                        Enemy2Script.e1currentHealth = Enemy2Script.e1currentHealth - pobrazeniakrytycznezredukowane;
                        PlayerBattle.pcurrkon = PlayerBattle.pcurrkon - 15;
                        info.text += "<Color=blue>" + "<b>" + "Wykonujesz atak i zadajesz uderzeniem krytycznym " + pobrazeniakrytycznezredukowane + " pkt obrażeń! (Przeciwnik zredukował gardą" + Enemy2Script.e1garde + "% obrażeń)" + "\n" + "</b>" + "</color>";
                    ...
                    else
                    {
                        ...
                        {
                            Enemy2Script.e1currentHealth = Enemy2Script.e1currentHealth - pobrazeniazredukowane;
                            PlayerBattle.pcurrkon = PlayerBattle.pcurrkon - 5;
                            info.text += "<Color=blue>" + "<b>" + "\n" + "Wykonujesz atak i zadajesz " + pobrazeniazredukowane + " pkt obrażeń! (Przeciwnik zredukował gardą " + Enemy2Script.e1garde + "% obrażeń)" + "\n" + "\n" + "</b>" + "</color>";
            ...
            else
            {
                ...
                if (Random.Range(1, 101) <= czykrytyk)
                {
                    ...
                    if (PlayerBattle.pcurrkon >= czymakondycje)
                    {
                        Enemy2Script.e1currentHealth = Enemy2Script.e1currentHealth - pobrazeniakrytyczne;
                        PlayerBattle.pcurrkon = PlayerBattle.pcurrkon - 15;
                    ...
                    else
                    {
                        if (PlayerBattle.pcurrkon >= 5)
                        {
                            Enemy2Script.e1currentHealth = Enemy2Script.e1currentHealth - pobrazenia;
                            PlayerBattle.pcurrkon = PlayerBattle.pcurrkon - 5;
    ...
    {
        eatak = 1;
        eszansatrafienia = rzut1k20 + E2ModifiScript.trafienie;
        info.text += "<Color=red>" + "Punkty trafienia przeciwnika: " + rzut1k20 + "+" + E2ModifiScript.trafienie + "=" + eszansatrafienia + "\n" + "</color>";
     ...
     public void EAtak()
    {
        ...
        if (etrafienie == 1)
        {
            ...
            if (pwylosowanaobrona == "Garda")
            {
                eobrazenia = (int)(E2ModifiScript.obrazenia - PModifiScript.wytrzymalosc);
                if (eobrazenia <= 0)
                {
                    eobrazenia = 0;
                }
                ...
                if (Random.Range(1, 101) <= czykrytyk)
                {
                    ...
                    czyemakondycje = Enemy2Script.e1currentKon * 0.03f;
                    if (Enemy2Script.e1currentKon >= czyemakondycje)
                    {
                        PlayerBattle.pcurrhealth = PlayerBattle.pcurrhealth - eobrazeniakrytycznezredukowane;
                        Enemy2Script.e1currentKon = Enemy2Script.e1currentKon - 15;
                    ...
                    {
                        if (Enemy2Script.e1currentKon >= 5)
                        {
                            PlayerBattle.pcurrhealth = PlayerBattle.pcurrhealth - eobrazeniazredukowane;
                            Enemy2Script.e1currentKon = Enemy2Script.e1currentKon - 5;
            ...
            else
            {
                ...
                if (Random.Range(1, 101) <= czykrytyk)
                {
                    float czymakondycje = Enemy2Script.e1currentKon * 0.03f;
                    if (Enemy2Script.e1currentKon >= czymakondycje)
                    {
                    ...
                    if (ewylosowanaobrona == "Garda")
                    {
                        ...
                        if (Random.Range(1, 101) <= czykrytyk)
                        {
                            ...
                            if (PlayerBattle.pcurrkon >= czymakondycje)
                            {
                                Enemy2Script.e1currentHealth = Enemy2Script.e1currentHealth - pobrazeniakrytycznezredukowane;
                                PlayerBattle.pcurrkon = PlayerBattle.pcurrkon - 15;
                                info.text += "<Color=blue>" + "<b>" + "Wykonujesz kontratak i zadajesz uderzeniem krytycznym " + pobrazeniakrytycznezredukowane + " pkt obrażeń! (Przeciwnik zredukował gardą" + Enemy2Script.e1garde + "% obrażeń)" + "\n" + "</b>" + "</color>";
                                Debug.Log("Gracz zadaje " + pobrazeniakrytycznezredukowane + " pkt obrażeń! (redukcja gardą)");
                            }
                            else
                            {
                                if (PlayerBattle.pcurrkon >= 5)
                                {
                                    Enemy2Script.e1currentHealth = Enemy2Script.e1currentHealth - pobrazeniazredukowane;
                                    PlayerBattle.pcurrkon = PlayerBattle.pcurrkon - 5;
                                    info.text += "<Color=blue>" + "<b>" + "\n" + "Wykonujesz kontratak i zadajesz " + pobrazeniazredukowane + " pkt obrażeń! (Przeciwnik zredukował gardą " + Enemy2Script.e1garde + "% obrażeń)" + "\n" + "\n" + "</b>" + "</color>";
                                    Debug.Log("Gracz zadaje " + pobrazeniazredukowane + " pkt obrażeń z kontrataku! (redukcja gardą)");
                                }
                                else
                                {
    ...
    public void EKontratak()
    {
        if (etrafienie == 1)
        {
            if (ewylosowanaobrona == "Parowanie")
            {
                if ((int)Random.Range(1, 101) <= Enemy2Script.e1counter)
                {
                    if (pwylosowanaobrona == "Garda")
                    {
                        eobrazenia = (int)(E2ModifiScript.obrazenia - PModifiScript.wytrzymalosc);
    ...
    public void RegeneracjaKondycji()
    {
        ...
        Enemy2Script.e1currentKon = Enemy2Script.e1currentKon + E2ModifiScript.conditionreg;
